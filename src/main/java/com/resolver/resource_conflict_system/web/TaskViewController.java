...
public class TaskViewController {
    ...
    private final com.resolver.resource_conflict_system.service.AssignmentService assignmentService;

    public TaskViewController(ResourceCatalogService resourceCatalogService, AssetCatalogService assetCatalogService,
                              SchedulerEngine schedulerEngine, BankersSafetyService bankersSafetyService, UserRepository userRepository, com.resolver.resource_conflict_system.service.AssignmentService assignmentService) {
    ...
    public String newTask(Model model) {
        ...
        model.addAttribute("assetOptions", buildAssetOptions());
        return "task-form";
    }
    ...
    public String validateTask(@RequestParam String id, Model model) {
        ...
        if (advice.currentSafe()) {
            // persist assignments from the simulated schedule
            assignmentService.persistAssignments(report.assignments());
    ...
    private List<AssetOption> buildAssetOptions() {
        var usageCounts = resourceCatalogService.approvedAssetUsageCounts();
        return assetCatalogService.snapshotAssets().stream()
                .map(asset -> {
                    int allocated = usageCounts.getOrDefault(asset.id(), 0).intValue();
                    int available = Math.max(0, asset.quantity() - allocated);
                    String label = asset.name() + " (available " + available + " / total " + asset.quantity() + ")";
                    return new AssetOption(asset.id(), label);
                })
                .toList();
    }

    public record AssetOption(String id, String label) {
    }
